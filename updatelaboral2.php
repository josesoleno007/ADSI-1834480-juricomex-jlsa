<?php
    ...
    session_start();
        ...
        try {
              ...
              $stmt = $conn->prepare("UPDATE laborales2 SET fechapublicacion = ?, radicacion = ?, demandante = ?, demandado = ?, juzgado = ?, tproceso = ?, tema = ?, enlace = ? WHERE id = ?");
              $stmt->execute(array($fechapublic, $radicacion, $dte, $ddo, $juzgado, $proceso, $tema, $enlace, $id));
             header('location: laborales2.php');
